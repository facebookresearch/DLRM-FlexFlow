 ...
 */
 ...
 void FFModel::sigmoid_cross_entropy_loss(const std::string& pcname,
                        const Tensor& _logit,
                        const Tensor& _label,
                        const std::string& reduction)
 {
   ...
   BinaryCrossEntropyLoss* op = new BinaryCrossEntropyLoss(*this, pcname, _logit, _label, aggr);
   layers.push_back(op);
 }
 
 BinaryCrossEntropyLoss::BinaryCrossEntropyLoss(FFModel& model,
 ...
 void BinaryCrossEntropyLoss::init(const FFModel& model)
 {}
 
 void BinaryCrossEntropyLoss::forward(const FFModel& model)
 {
 }
 ...
 __host__
 PerfMetrics BinaryCrossEntropyLoss::backward_task(const Task *task,
                                    const std::vector<PhysicalRegion>& regions,
                                    Context ctx, Runtime* runtime)
 {
   assert(regions.size() == 3);
   assert(task->regions.size() == 3);
   const BinaryCrossEntropyLoss* op = (BinaryCrossEntropyLoss*) task->args;
   ...
   if (op->profiling) {
     print_tensor<2, float>(accLabels.ptr, accLabels.rect, "[BinaryCrossEntropyLoss:label]");
     print_tensor<2, float>(accLogits.ptr, accLogits.rect, "[BinaryCrossEntropyLoss:logit]");
     print_tensor<2, float>(accLogitsGrad.ptr, accLogitsGrad.rect, "[BinaryCrossEntropyLoss:logit_grad]");
   }
   return perf_zc;
 }
 
 void BinaryCrossEntropyLoss::backward(const FFModel& model)
 {
   ArgumentMap argmap;
   Context ctx = model.config.lg_ctx;
   Runtime* runtime = model.config.lg_hlr;
   IndexLauncher launcher(MSELOSS_BWD_TASK_ID, task_is,
                          TaskArgument(this, sizeof(BinaryCrossEntropyLoss)), argmap,
