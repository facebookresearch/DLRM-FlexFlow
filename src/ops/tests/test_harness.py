...
class LinearWithSigmoid(torch.nn.Module):
    def __init__(self, i_dim, o_dim, weights=None, bias=False):
        super(LinearWithSigmoid, self).__init__()
        self.linear_layer = torch.nn.Linear(i_dim, o_dim, bias=bias)
        if weights is not None:
            assert len(weights) == 2
            self.linear_layer.weight = torch.nn.Parameter(torch.from_numpy(weights[0]).float(), requires_grad=True)
            if bias:
                self.linear_layer.bias = torch.nn.Parameter(torch.from_numpy(weights[1]).float(), requires_grad=True)
    
    def forward(self, layer_input):
        logits = self.linear_layer(layer_input)
        output = torch.sigmoid(logits)
        return output


class LinearLogLossWithLogitSelfReferenceTest(unittest.TestCase):
    TEST_TARGET = 'sigmoid_ce_with_logit_test'
    TEST_TARGET2 = 'sigmoid_ce_with_logit_2_test'
    ...
    def _run_gpu_test(self, num_gpu, i_dim, o_dim, i_shape, \
        ...
        # generate FF results
        gen_FF_result(LinearLogLossWithLogitSelfReferenceTest.TEST_TARGET, num_gpu)
        # generate the other FF refernce results
        gen_FF_result(LinearLogLossWithLogitSelfReferenceTest.TEST_TARGET2, num_gpu)
...
class LinearLogLossWithLogitTest(unittest.TestCase):
    ...
    def _run_gpu_test(self, num_gpu, i_dim, o_dim, i_shape, \
        ...
        # generate FF results
        gen_FF_result(LinearLogLossWithLogitTest.TEST_TARGET, num_gpu)
...
class DotCompressorTest(unittest.TestCase):
    ...
    def _run_gpu_test(self, num_gpu, batch_size, i_dim, \
                      ...
                      dense_projection_i_dim, epsilon=0.01, \
        ...
        sparse_embedding = np.random.uniform(0, 1, (batch_size, i_dim))
        ...
        # print('chunk dense embedded', chunk_dense_embedded.shape, "chunk sparse embedded", chunk_sparse_embedded.shape)
        pretrained_weights = [linear_weight, linear_bias]
        m = DotCompressor(num_channels, projected_num_channels, pretrained_weights, bias=False)
        if not concat_last:
        ...
        for layer_id in layer_outputs:
          ...
          # is_equal_tensor_from_file(file1, file2, layer_id, epsilon=epsilon)
