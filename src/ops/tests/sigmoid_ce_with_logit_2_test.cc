...
#define output_dim 2
  ...
  Tensor actvation_out = ff.sigmoid<output_dim>("", linear_out, o_dims);
  ...
  ff.cross_entropy_loss<output_dim>("", actvation_out, labels);
  ...
  dump_region_to_file(ff, ff.parameters[0].tensor.region, "kernel_updated2.txt", 2);
#undef input_dim
#undef output_dim
}
