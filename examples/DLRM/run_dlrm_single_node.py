...
with open("res_2_nodes_new.csv", "w") as csvfile:
    ...
    for top_c in TOP_CONFIGS:
        ...
        for gpu_batch_size, gpus in itertools.product(per_gpu_batch_sizes, num_gpus):
            for _ in range(repeat):
                strategy = "../../src/runtime/dlrm_strategy_emb_{}_gpu_{}_node_{}.pb".format(NUM_EMBEDDING, gpus, NODE)
                ...
                cmd = (
                    ...
                    data_size=batch_size,  # make sure we have data for 4 iterations
                    gpus=gpus,
                    strategy=strategy,
                ...
                try:
                    gen_strategy_cmd = 'cd ../../src/runtime && ./gen_strategy.sh %d %d %d' % (gpus, NUM_EMBEDDING, NODE)
